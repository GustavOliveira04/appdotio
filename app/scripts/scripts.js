let rowCount = 0;
  ...
  function formatNum(n){
    return truncate(n, 3).toLocaleString('pt-BR', {minimumFractionDigits:3, maximumFractionDigits:3});
  }
  ...
  function applyDecimalMask(input){
    ...
    input.value = parseInt(intPart, 10) + ',' + decPart;
  }
  ...
  function addRow(){
    ...
    card.querySelectorAll('.bordo-pos, .eixo, .bordo-neg').forEach(inp => {
      inp.addEventListener('input', () => {
        applyDecimalMask(inp);
        calculateAll();
      });
    });
